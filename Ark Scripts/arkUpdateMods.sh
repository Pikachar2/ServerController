...
if  !(pgrep -x "ShooterGameServ"); then
	...
	steamModDir="$STEAM_LOCATION/"
	baseDir="$ARK_SERVER_DIR/"
#	modDir="${steamModDir}steamapps/workshop/extracted/"
	...
	modArray=(`find "$ARK_SERVER_DIR/ShooterGame/Content/Mods/" -maxdepth 1 -type d -and ! -name '*[!0-9]*' -and ! -name '111111111' | cut -c 56-`)
...
#	for MOD in 801082391
	...
	do
		steamOutput=" "
		while [ true ]; do
			steamOutput=`$STEAM_CMD_LOCATION/steamcmd +login anonymous +force_install_dir "$steamModDir" +workshop_download_item 346110 $MOD validate +quit`
			echo "steamOutput"
			echo "$steamOutput"
		...
		$STEAM_CMD_LOCATION/steamcmd +login anonymous +force_install_dir "$steamModDir" +workshop_download_item 346110 $MOD validate +quit
